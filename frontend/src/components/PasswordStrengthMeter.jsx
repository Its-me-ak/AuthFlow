...
const PasswordStrengthMeter = ({ password }) => {
  const getStrengthLevel = (pass) => {
    ...
    const conditionsMet = [hasNumber, hasSymbol, hasUpper, hasLower].filter(
      Boolean
    ).length;
    ...
    <div className="mt-2">
      ...
      <div className="flex gap-1">
        {[...Array(5)].map((_, index) => (
          <div
            key={index}
            className={`h-1 w-full flex-1 rounded-full transition-colors duration-300 ${
              index < strengthLevel
                ? getColorByLevel(strengthLevel)
                : "bg-gray-400"
            }`}
          />
        ))}
